<?php require $_SERVER['DOCUMENT_ROOT'].'/key.php';?>
<?php
...
{
    $apiurl = "https://dailycode.vn/apii/key.php?api_key=" . urlencode($apiKey);
    $urlParts = parse_url($apiurl);
    $host = $urlParts["host"];
    if ($host !== "dailycode.vn") {
    ...
    if ($response === false) {
        die("Không thể kết nối đến dailycode.vn");
    }
    ...
    if ($httpCode === 200 && !empty($data)) {
        ...
        die("DaiLyCode từ chối kết nối, hoặc gặp lỗi nghiêm trọng, vui lòng liên hệ cho chủ sở hữu code này và gửi mã lỗi: `Server refused connection`");
    }
    return false;
}
...
if ($apiResult && isset($apiResult["results"][API_KEY])) {
    ...
    if ($status === "on") {
        ...
        {
            $allowedExtensions = array("gif", "jpg", "jpeg", "png", "ico", "webp");
        ...
        {
           $length = mb_strlen($name, "UTF-8"); // Sử dụng mb_strlen() cho chuỗi đa byte
        ...
        {
            $allowedExtensions = array("mp3", "wav", "flac", "aac");
            $urlInfo = pathinfo($url);
            $extension = isset($urlInfo["extension"]) ? strtolower($urlInfo["extension"]) : '';
            return in_array($extension, $allowedExtensions);
        }
        $zodiacs = array("Bạch Dương", "Cung Kim Ngưu", "Cung Song Tử", "Cung Cự Giải", "Cung Sư Tử", "Cung Xử Nữ", "Cung Thiên Bình", "Cung Hổ Cáp", "Cung Nhân Mã", "Cung Ma Kết", "Cung Bảo Bình", "Cung Song Ngư");
        $zodiacs_name = array("Bạch Dương (Aries) | 21/3 - 19/4", "Kim Ngưu (Taurus) | 20/4 - 20/5", "Song Tử (Gemini) | 21/5 - 20/6", "Cự Giải (Cancer) | 21/6 - 22/7", "Sư Tử (Leo) | 23/7 - 22/8", "Xử Nữ (Virgo) | 23/8 - 22/9", "Thiên Bình (Libra) | 23/9 - 22/10", "Hổ Cáp (Scorpio) | 23/10 - 21/11", "Nhân Mã (Sagittarius) | 22/11 - 21/12", "Ma Kết (Capricorn) | 22/12 - 19/1", "Bảo Bình (Aquarius) | 20/1 - 18/2", "Song Ngư (Pisces) | 19/2 - 20/3");
        function Confuscator($noidung)
        {
            $understand_dev = array("/<\\s*script[^>]*>.*?<\\s*\\/\\s*script\\s*>/is", "/<\\s*style[^>]*>.*?<\\s*\\/\\s*style\\s*>/is", "/<\\?php.*?\\?>/is", "/\\bphp\\b/i", "/\\bjavascript\\b/i", "/<\\//", "/<\\/>/", "/</", "/>/");
                        ...
                        <div class="row justify-content-center">
                            <div class="col-md-8 col-lg-6 col-xl-4 py-6">
                                <div class="text-center">
                                    ...
                                    <h2 class="h6 fw-normal text-muted mb-3" style="color:blue !important;">Thông báo: <?php 
        echo $reason ?? "không có dữ liệu";
        ?>
</h2>
